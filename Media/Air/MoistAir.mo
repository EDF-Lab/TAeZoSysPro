...
package MoistAir
  ...
  redeclare replaceable model extends BaseProperties(
    T(stateSelect=if preferredMediumStates then StateSelect.prefer else
          StateSelect.default),
    d(stateSelect=if preferredMediumStates then StateSelect.prefer else
          StateSelect.default),
...
required from medium model \""
  ...
  algorithm
    assert(
      if size(X, 1) == nX then X[Air] > 1e-4 else (1- X[Water]) > 1e-4,
      "Too little dry air in the mixture to compute the density",
      AssertionLevel.error);
    X_sat := k_mair/(p/min(saturationPressure(T), 0.999*p) - 1 + k_mair);
    X_steam := min(X_sat, X[Water]);
    state :=
      if size(X, 1) == nX then ThermodynamicState(
        d=p*(1-X_steam/(X_steam+k_mair*X[Air]))/(dryair.R* T * X[Air]),
        T=T,
        X=X)
      else ThermodynamicState(
        d=p*(1-X_steam/(X_steam+k_mair*(1-X[Water])))/(dryair.R* T * (1-X[Water])),
        T=T,
        X=cat(
          1,
          X,
  ...
  algorithm
    assert(
      p-saturationPressure(T) >= 1e-4,
      "p_sat(T) is too close to p",
      AssertionLevel.error);
    x_sat := k_mair*saturationPressure(T)/max(100*Modelica.Constants.eps, p -
  ...
  function gasConstant_X
    "Return ideal gas constant as a function from composition X (only valid for phi<1)"
    extends Modelica.Icons.Function;
  ...
  algorithm
    ...
    annotation (smoothOrder=2, Documentation(info=
    "<html>
...
<a href=\"modelica://Modelica.Media.Air.MoistAir.sublimationPressureIce\">solid</a> and the <a href=\"modelica://Modelica.Media.Air.MoistAir.saturationPressureLiquid\"> liquid</a> region, respectively, are combined using the first derivative continuous <a href=\"modelica://Modelica.Media.Air.MoistAir.Utilities.spliceFunction\">spliceFunction</a>. This functions range of validity is from 190 to 647.096 K. For more information on the type of correlation used, see the documentation of the linked functions.
  ...
  replaceable function saturationPressure_der
  ...
  replaceable function saturationPressureLiquid
  ...
  replaceable function saturationPressureLiquid_der
  ...
  replaceable function sublimationPressureIce
  ...
  replaceable function sublimationPressureIce_der
  ...
  replaceable function enthalpyOfLiquid
  ...
  algorithm
    h := steam.cp * (T - 273.15) * X[Water] + dryair.cp * (T - 273.15) * (1.0 - X[Water]);
    annotation (
      Inline=true,
      smoothOrder=5,
      Documentation(info="<html>
  ...
  replaceable function enthalpyOfNonCondensingGas
  ...
  function h_dTX
    "Return specific enthalpy of moist air as a function of density d, temperature T and composition X"
    extends Modelica.Icons.Function;
  ...
  Modelica.Media.Water.IF97_Utilities.BaseIF97.Basic.psat_der(T, dT);
    ...
    h_der := X_steam*steam.cp*dT + dX_steam*steam.cp*(T-273.15) + X_air*dryair.cp*dT
      + dX_air*dryair.cp*(T-273.15) + X_liquid*enthalpyOfWater_der(T=T, dT=dT) + dX_liq*enthalpyOfWater(T);
  ...
  algorithm
    ...
    /* h        := {SingleGasNasa.h_Tlow(data=steam,  T=T, refChoice=ReferenceEnthalpy.UserDefined, h_off=46479.819+2501014.5),
                 SingleGasNasa.h_Tlow(data=dryair, T=T, refChoice=ReferenceEnthalpy.UserDefined, h_off=25104.684)}*
      {X_steam, X_air} + enthalpyOfLiquid(T)*X_liquid;*/
    h := (X_steam*steam.cp + X_air*dryair.cp)*(T-273.15) + enthalpyOfWater(T)*X_liquid;
    annotation (
  ...
  algorithm
    ...
    h_der := X_steam*steam.cp*dT + dX_steam*steam.cp*(T-273.15)
      + X_air*dryair.cp*dT + dX_air*dryair.cp*(T-273.15)
  ...
  algorithm
    h := {steam.cp*(state.T-273.15), dryair.cp*(state.T-273.15)}*state.X;

    p1 := state.d*(state.X[Water]*steam.R + (1-state.X[Water])*dryair.R) * state.T;

    h_is := h + gamma/(gamma - 1.0)*(state.T*gasConstant_X(state.X))*((p2/p1)
      ^((gamma - 1)/gamma) - 1.0);
  ...
  function specificInternalEnergy_dTX
    "Return specific internal energy of moist air as a function of density d, temperature T and composition X"
    extends Modelica.Icons.Function;
    input SI.Density d "Density";
    input SI.Temperature T "Temperature";
    input SI.MassFraction X[:] "Mass fractions of moist air";
  ...
  algorithm
    ...
    u := X_steam*steam.cp*(T-273.15) + X_air*dryair.cp*(T-273.15) + enthalpyOfWater(T)*X_liquid - R_gas*T;

    annotation (derivative=specificInternalEnergy_dTX_der, Documentation(info=
           "<html>
  ...
  function specificInternalEnergy_dTX_der
  ...
  Modelica.Media.Water.IF97_Utilities.BaseIF97.Basic.psat_der(T, dT);
    ...
    u_der := X_steam*steam.cp*dT + dX_steam*steam.cp*(T-273.15) + X_air*dryair.cp*dT + dX_air*dryair.cp(T-273.15) + X_liquid*enthalpyOfWater_der(T=T, dT=dT) +
  ...
  function s_dTX
    "Return specific entropy of moist air as a function of density d, temperature T and composition X"
    extends Modelica.Icons.Function;
    input SI.Density d "Density";
    input SI.Temperature T "Temperature";
    input SI.MassFraction X[:] "Mass fractions of moist air";
  ...
  algorithm
    ...
    s:= (1 - X[Water])*dryair.cp*Modelica.Math.log(T/273.15)
    + X[Water]*steam.cp*Modelica.Math.log(T/273.15)
    ...
    + X_liquid * water.cp * Modelica.Math.log(T / 273.15);
    annotation (
  ...
  Modelica.Media.Water.IF97_Utilities.BaseIF97.Basic.psat_der(T, dT);
    ...
    ds := (1 - X[Water]) * dryair.cp*dT/T + dX_air * dryair.cp * Modelica.Math.log(T/273.15)
          + X[Water] * steam.cp*dT/T + dX[Water] * Modelica.Math.log(T/273.15)
          - Modelica.Constants.R*(1/MMX[Water]*(Utilities.smoothMax_der(
            ...
            dX[Water],
            0.0,
            0.0)*(Modelica.Math.log(max(dp[Water], Modelica.Constants.eps)/reference_p)) + dp[Water]/p[Water]*Utilities.smoothMax(
            X[Water],
            0.0,
            ...
            dX[Air],
            0.0,
            0.0)*(Modelica.Math.log(max(p[Air], Modelica.Constants.eps)/reference_p)) + dp[Air]/p[Air]*Utilities.smoothMax(
            X[Air],
            0.0,
            1e-9)))
          + dX_liq * water.cp * Modelica.Math.log(T / 273.15) + X_liquid * water.cp * dT/T;
  ...
  algorithm
    eta := 1e-6*Polynomials_Temp.evaluateWithRange(
        ...
        Modelica.SIunits.Conversions.to_degC(123.15),
        Modelica.SIunits.Conversions.to_degC(1273.15),
