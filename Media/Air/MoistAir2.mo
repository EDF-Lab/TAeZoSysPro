...
<p>2012-01-12        Stefan Wischhusen: Initial Release.</p>
  ...
  algorithm
    ...
    + Utilities.smoothMax((1 - X[Water])/MMX[Air],0.0,1e-9)*Modelica.Math.log(max(p[Air], Modelica.Constants.eps)/reference_p)) + X_liquid * 4181.6 * (Modelica.Math.log(reference_T / 273.15) + Modelica.Math.log(T / reference_T));
    annotation (
...
Specific entropy of moist air is computed from density, temperature and composition with X[1] as the total water mass fraction.
  ...
  Modelica.Media.Water.IF97_Utilities.BaseIF97.Basic.psat_der(T, dT);
    ...
    ds := Modelica.Media.IdealGases.Common.Functions.s0_Tlow_der(
      dryair,
      T,
      dT)*(1 - X[Water]) + Modelica.Media.IdealGases.Common.Functions.s0_Tlow_der(
      steam,
      T,
      ...
      X[Air],
      0.0,
      1e-9))) + dX_liq * 4181.6 * (Modelica.Math.log(reference_T / 273.15) + Modelica.Math.log(T / reference_T)) + X_liquid * 4181.6 * (dT / T);
