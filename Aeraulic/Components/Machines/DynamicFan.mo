...
model DynamicFan
  ...
  parameter Modelica.SIunits.AngularVelocity w0(displayUnit = "rev/min") = 157.0796 "Nominal fan speed";
  //
  ...
  Modelica.SIunits.PressureDifference dp0 "Pressure difference at nominal speed";
  ...
  Modelica.SIunits.VolumeFlowRate Qv0(start = Table[integer(N / 2), 1], displayUnit = "m3/h") "Volume flow rate at nominal speed";
  Modelica.SIunits.AngularVelocity w(displayUnit = "rev/min") "fan speed";
  //
  Modelica.Fluid.Interfaces.FluidPort_a fluidport_a(replaceable package Medium = Medium) annotation(
    Placement(visible = true, transformation(origin = {-98, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {-94, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Modelica.Fluid.Interfaces.FluidPort_b fluidport_b(replaceable package Medium = Medium) annotation(
    Placement(visible = true, transformation(origin = {100, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {100, 84}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Modelica.Mechanics.Rotational.Interfaces.Flange_a flange annotation(
    Placement(visible = true, transformation(origin = {2, 2}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {4, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
equation
  d = if m_flow >= 0.0 then medium_a.d else medium_a.d;
  Qv_flow = m_flow / d;
//momentum balance
  dp = fluidport_a.p - fluidport_b.p;
  dp0 + sum({Coeff[j] .* Qv0 ^ (j - 1) for j in 1:OrderPolyFitting + 1}) = 0.0;
  dp / dp0 = (w / w0) ^ 2.0;
  Qv_flow / Qv0 = w / w0;
//Mechanical balance
  w = der(flange.phi);
