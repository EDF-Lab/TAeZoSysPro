...
model AeraulicConvection
  // Media
  replaceable package Medium = TAeZoSysPro.HeatTransfer.Media.MyMedia;
  package MediumAeraulic = TAeZoSysPro.Aeraulic.Media.MyMedia;
  ...
  //
  parameter Modelica.SIunits.Height carac_length = 0 "charactéritic length";
  //
  ...
  // Variables
  Modelica.SIunits.Temperature meanT;
  // Mean temperature
  ...
  TAeZoSysPro.Aeraulic.Interfaces.FlowPort_a flowPort_Inlet(replaceable package Medium = MediumAeraulic) annotation(
    Placement(visible = true, transformation(origin = {100, -4}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {100, 2}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  TAeZoSysPro.Aeraulic.Interfaces.FlowPort_b flowPort_OutletUp(replaceable package Medium = MediumAeraulic) annotation(
    Placement(visible = true, transformation(origin = {0, 84}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {0, 84}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  TAeZoSysPro.Aeraulic.Interfaces.FlowPort_b flowPort_OutletDown(replaceable package Medium = MediumAeraulic) annotation(
    Placement(visible = true, transformation(origin = {-4, -92}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {0, -92}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Modelica.SIunits.MassFlowRate m_flow "Convection flow kg/s";
protected
  Medium.ThermodynamicState state;
  Modelica.SIunits.MassFraction[MediumAeraulic.nX] X_Inlet "Mass fraction vector at port inlet";
initial equation
// Energy at time 0 second is equal to 0J
  E = 0;
equation
  X_Inlet = 1 / sum(flowPort_Inlet.di) * flowPort_Inlet.di;
// Temperature Calculation
  meanT = (Heatport_a.T + flowPort_Inlet.T) / 2;
  deltaT = Heatport_a.T - flowPort_Inlet.T;
// state vector
...
// Correlation selection
    if correlation_free == TAeZoSysPro.HeatTransfer.Types.FreeConvectionCorrelation.ChurchillAndChu_vertical_plate then
      hcv = TAeZoSysPro.HeatTransfer.Functions.FreeConvection.ChurchillAndChu_Vert_Plate(flowPort_Inlet.T, deltaT, carac_length, d, cp, eta, k, pr);
    ...
    elseif correlation_free == TAeZoSysPro.HeatTransfer.Types.FreeConvectionCorrelation.Cibse_vertical_plate then
      hcv = TAeZoSysPro.HeatTransfer.Functions.FreeConvection.Cibse_Vert_Plate(deltaT, flowPort_Inlet.T, carac_length, d, cp, eta, k);
    elseif correlation_free == TAeZoSysPro.HeatTransfer.Types.FreeConvectionCorrelation.Ground then
      hcv = TAeZoSysPro.HeatTransfer.Functions.FreeConvection.Ground(Heatport_a.T, flowPort_Inlet.T, deltaT, meanT, perimeter, d, cp, eta, k, A);
    elseif correlation_free == TAeZoSysPro.HeatTransfer.Types.FreeConvectionCorrelation.Ceiling then
      hcv = TAeZoSysPro.HeatTransfer.Functions.FreeConvection.Ceiling(Heatport_a.T, flowPort_Inlet.T, deltaT, meanT, perimeter, d, cp, eta, k, A);
    elseif correlation_free == TAeZoSysPro.HeatTransfer.Types.FreeConvectionCorrelation.ChurchillAndChu_horizontal_cylinder then
      hcv = TAeZoSysPro.HeatTransfer.Functions.FreeConvection.ChurchillAndChu_Hor_Cyl(flowPort_Inlet.T, deltaT, meanT, carac_length, d, cp, eta, k, pr);
    elseif correlation_free == TAeZoSysPro.HeatTransfer.Types.FreeConvectionCorrelation.Morgan_horizontal_cylinder then
      hcv = TAeZoSysPro.HeatTransfer.Functions.FreeConvection.Morgan_Hor_Cyl(flowPort_Inlet.T, deltaT, meanT, carac_length, d, cp, eta, k, pr);
...
//Aeraulic Calculation
  ...
  flowPort_OutletUp.m_flow = -m_flow * TAeZoSysPro.Aeraulic.Functions.regStep(x = deltaT, x_small = 0.01, y1 = X_Inlet, y2 = {0, 0});
  flowPort_Inlet.m_flow + flowPort_OutletUp.m_flow + flowPort_OutletDown.m_flow = fill(0.0, MediumAeraulic.nX);
  flowPort_Inlet.H_flow = m_flow * flowPort_Inlet.h;
  flowPort_OutletUp.H_flow = -(m_flow * flowPort_Inlet.h + Q_flow) * TAeZoSysPro.Aeraulic.Functions.regStep(x = deltaT, x_small = 0.01, y1 = 1, y2 = 0);
  flowPort_Inlet.H_flow + flowPort_OutletUp.H_flow + flowPort_OutletDown.H_flow + Q_flow = 0;
  annotation(
    Icon(graphics = {Rectangle(origin = {-54, 0}, fillColor = {140, 138, 145}, fillPattern = FillPattern.Cross, extent = {{-26, 100}, {26, -100}}), Line(points = {{0, 80}, {0, -80}}, color = {0, 85, 255}, thickness = 1, arrow = {Arrow.None, Arrow.Filled}), Line(origin = {40, 0}, points = {{0, 80}, {0, -80}}, color = {0, 85, 255}, thickness = 1, arrow = {Arrow.None, Arrow.Filled}), Line(origin = {80, 0}, points = {{0, 80}, {0, -80}}, color = {0, 85, 255}, thickness = 1, arrow = {Arrow.None, Arrow.Filled}), Line(origin = {35, 40}, points = {{-55, 0}, {55, 0}}, color = {255, 0, 0}, thickness = 1, arrow = {Arrow.None, Arrow.Filled}), Line(origin = {35, -40}, points = {{-55, 0}, {55, 0}}, color = {255, 0, 0}, thickness = 1, arrow = {Arrow.None, Arrow.Filled}), Text(origin = {36, 4}, extent = {{-28, 24}, {28, -24}}, textString = "hcv"), Line(origin = {35, 0}, points = {{-27, -22}, {27, 22}}, thickness = 1.75, arrow = {Arrow.None, Arrow.Filled}), Text(origin = {-94, 29}, rotation = -90, extent = {{-28, 5}, {28, -5}}, textString = "Wall"), Text(origin = {92, 27}, rotation = 90, extent = {{-28, 5}, {28, -5}}, textString = "Air")}, coordinateSystem(initialScale = 0.1)),
