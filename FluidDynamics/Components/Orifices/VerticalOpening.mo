...
protected
  ...
  parameter Modelica.SIunits.Height H_fluidSteam = H * Cd ^0.5 "Minimal height between top and bottom flow path";
...
//
  ...
  for i in 1:N loop  
    dp_i[i] = dp + Modelica.Constants.g_n * (H_fluidSteam / 2 -H_fluidSteam * (i - 1 / 2) / N) * (sum(port_a.d) - sum(port_b.d));
