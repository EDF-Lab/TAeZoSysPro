...
model ConvectionHood
  ...
  Modelica.Thermal.HeatTransfer.Interfaces.HeatPort_b Heatport_b annotation(
    Placement(visible = true, transformation(origin = {100, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {100, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Modelica.Blocks.Interfaces.RealInput TairInlet annotation(
    Placement(visible = true, transformation(origin = {8, -84}, extent = {{-20, -20}, {20, 20}}, rotation = 0), iconTransformation(origin = {20, 82}, extent = {{-16, -16}, {16, 16}}, rotation = -90)));
  // Variables
...
// Temperature Calculation
  meanT = (Heatport_a.T + TairInlet) / 2;
  deltaT = Heatport_a.T - TairInlet;
// state vector
...
// Correlation selection
    if correlation_free == TAeZoSysPro.HeatTransfer.Types.FreeConvectionCorrelation.ChurchillAndChu_vertical_plate then
      hcv = TAeZoSysPro.HeatTransfer.Functions.FreeConvection.ChurchillAndChu_Vert_Plate(TairInlet, deltaT, carac_length, d, cp, eta, k, pr);
    ...
    elseif correlation_free == TAeZoSysPro.HeatTransfer.Types.FreeConvectionCorrelation.Cibse_vertical_plate then
      hcv = TAeZoSysPro.HeatTransfer.Functions.FreeConvection.Cibse_Vert_Plate(deltaT, TairInlet, carac_length, d, cp, eta, k);
    elseif correlation_free == TAeZoSysPro.HeatTransfer.Types.FreeConvectionCorrelation.Ground then
      hcv = TAeZoSysPro.HeatTransfer.Functions.FreeConvection.Ground(Heatport_a.T, TairInlet, deltaT, meanT, perimeter, d, cp, eta, k, A);
    elseif correlation_free == TAeZoSysPro.HeatTransfer.Types.FreeConvectionCorrelation.Ceiling then
      hcv = TAeZoSysPro.HeatTransfer.Functions.FreeConvection.Ceiling(Heatport_a.T, TairInlet, deltaT, meanT, perimeter, d, cp, eta, k, A);
    elseif correlation_free == TAeZoSysPro.HeatTransfer.Types.FreeConvectionCorrelation.ChurchillAndChu_horizontal_cylinder then
      hcv = TAeZoSysPro.HeatTransfer.Functions.FreeConvection.ChurchillAndChu_Hor_Cyl(TairInlet, deltaT, meanT, carac_length, d, cp, eta, k, pr);
    elseif correlation_free == TAeZoSysPro.HeatTransfer.Types.FreeConvectionCorrelation.Morgan_horizontal_cylinder then
      hcv = TAeZoSysPro.HeatTransfer.Functions.FreeConvection.Morgan_Hor_Cyl(TairInlet, deltaT, meanT, carac_length, d, cp, eta, k, pr);
...
// Heatport
  ...
  annotation(
    Icon(graphics = {Rectangle(origin = {-54, 0}, fillColor = {140, 138, 145}, fillPattern = FillPattern.Cross, extent = {{-26, 100}, {26, -100}}), Line(points = {{0, 80}, {0, -80}}, color = {0, 85, 255}, thickness = 1, arrow = {Arrow.None, Arrow.Filled}), Line(origin = {40, 0}, points = {{0, 80}, {0, -80}}, color = {0, 85, 255}, thickness = 1, arrow = {Arrow.None, Arrow.Filled}), Line(origin = {80, 0}, points = {{0, 80}, {0, -80}}, color = {0, 85, 255}, thickness = 1, arrow = {Arrow.None, Arrow.Filled}), Line(origin = {35, 40}, points = {{-55, 0}, {55, 0}}, color = {255, 0, 0}, thickness = 1, arrow = {Arrow.None, Arrow.Filled}), Line(origin = {35, -40}, points = {{-55, 0}, {55, 0}}, color = {255, 0, 0}, thickness = 1, arrow = {Arrow.None, Arrow.Filled}), Text(origin = {36, 4}, extent = {{-28, 24}, {28, -24}}, textString = "hcv"), Line(origin = {35, 0}, points = {{-27, -22}, {27, 22}}, thickness = 1.75, arrow = {Arrow.None, Arrow.Filled}), Text(origin = {70, -93}, extent = {{-28, 5}, {28, -5}}, textString = "KURY-EDVANCE"), Text(origin = {-94, 29}, rotation = -90, extent = {{-28, 5}, {28, -5}}, textString = "Wall"), Text(origin = {92, 27}, rotation = 90, extent = {{-28, 5}, {28, -5}}, textString = "Air")}, coordinateSystem(initialScale = 0.1)),
