...
model Pipe
  /*
                                                                        ...
                                                                        */
  package Medium_external = Media.MyMedia;
  ...
  parameter Modelica.SIunits.Radius Ri_tube = 0 "Internal tube radius" annotation(
    Dialog(group = "tube capacity properties"));
  parameter Modelica.SIunits.Thickness e_tube = 0 "External tube radius" annotation(
    Dialog(group = "tube capacity properties"));
  // for PartialWall_Cylindrical : material = MineralFibre ( library BuildSysPro => Utilities => Data => Solid )
  parameter Integer n = 5 "Number of layers : 1 to 65535" annotation(
  ...
  parameter Modelica.SIunits.ThermalConductivity k_Insu = 0.036 "Insulation conductivity" annotation(
    Dialog(group = "Insulation properties"));
  parameter Modelica.SIunits.Thickness e_Insu = 0 "ExternalRadius : m" annotation(
  ...
  parameter TAeZoSysPro.HeatTransfer.Types.FreeConvectionCorrelation correlation_external = TAeZoSysPro.HeatTransfer.Types.FreeConvectionCorrelation.ChurchillAndChu_horizontal_cylinder "Free convection Correlation" annotation(
  ...
  TAeZoSysPro.HeatTransfer.BasesClasses.PartialWall D_Insulation(conduction = TAeZoSysPro.HeatTransfer.Types.ConductionType.Radial, CpW = CpTube, Le = Le, RhoW = RhoTube, l = if not Insulation then 1 else e_Insu, Ri = Ri_tube + e_tube, SteadyState = if not Insulation then false else SteadyState, Tinit = Tinit, add_on = 1, k = k_Insu, n = n) annotation(
  ...
  TAeZoSysPro.HeatTransfer.BasesClasses.Convection B_External_convection(redeclare package Medium = Medium_external, FreeConvection = true, A = A_external, carac_length = if not Insulation then 2 * (Ri_tube + e_tube) else 2 * (Ri_tube + e_tube + e_Insu), add_on = add_on_external_convection, correlation_free = correlation_external, h = h_external) annotation(
    Placement(visible = true, transformation(origin = {35, 65}, extent = {{-15, -15}, {15, 15}}, rotation = 90)));
  ...
  // Port variables
  Modelica.Thermal.HeatTransfer.Interfaces.HeatPort_a port_in annotation(
  ...
  Modelica.Thermal.HeatTransfer.Interfaces.HeatPort_b port_out annotation(
  ...
  Modelica.Thermal.HeatTransfer.Interfaces.HeatPort_b port_rad annotation(
    Placement(visible = true, transformation(origin = {-70, 88}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {-70, 86}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  ...
  Modelica.Blocks.Interfaces.RealOutput Awall = A_external annotation(
  ...
  Modelica.SIunits.Temperature T_Fluid_out "Outlet temperature of water of the pipe";
protected
  parameter Modelica.SIunits.Area CrossSection_tube = Modelica.Constants.pi * ((Ri_tube + e_tube) ^ 2 - Ri_tube ^ 2) "Tube area (cross section)";
  parameter Modelica.SIunits.Mass MassTube = RhoTube * CrossSection_tube * Le "Mass of tube";
  parameter Modelica.SIunits.Area A_external = if Insulation then 2 * Modelica.Constants.pi * (Ri_tube + e_tube + e_Insu) * Le else 2 * Modelica.Constants.pi * (Ri_tube + e_tube) * Le "External area of the insulation/tube ";
...
equation
  ...
  connect(port_conv, B_External_convection.Heatport_b) annotation(
    Line(points = {{70, 88}, {34, 88}, {34, 80}, {36, 80}}, color = {191, 0, 0}));
  connect(port_rad, C_External_radiation.port_b) annotation(
    Line(points = {{-70, 88}, {-34, 88}, {-34, 80}, {-34, 80}}, color = {191, 0, 0}));
  connect(Fview, C_External_radiation.Fview) annotation(
...
// equipotentiality
  B_Internal_convection.Heatport_b.T = 0.5 * (port_in.T + T_Fluid_out);
// energy conservation
  Qv * B_Internal_convection.d * B_Internal_convection.cp * (port_in.T - T_Fluid_out) - A_Internal_convection_power.Q_flow = 0;
// Internal_convection_power.Q_flow > 0 with the convention of the component internal convention
  port_in.Q_flow = 0;
  port_out.Q_flow = Qv * B_Internal_convection.d * B_Internal_convection.cp * (port_out.T - T_Fluid_out);
// pipe comments
  annotation(
    Icon(graphics = {Rectangle(origin = {11, 31}, fillColor = {255, 255, 255}, fillPattern = FillPattern.Backward, extent = {{-91, 29}, {69, -13}}), Rectangle(origin = {-30, -26}, fillColor = {255, 255, 255}, fillPattern = FillPattern.Backward, extent = {{-50, 10}, {110, -30}}), Text(origin = {-71, 69}, extent = {{-15, 5}, {15, -5}}, textString = "CarrollNode",  fontSize = 0 ), Text(origin = {70, 68}, extent = {{-8, 4}, {8, -4}}, textString = "Air",  fontSize = 0 ), Text(origin = {67, -73}, extent = {{-13, 5}, {13, -5}}, textString = "Fview",  fontSize = 0 ), Text(origin = {-70, -74}, extent = {{-10, 6}, {10, -6}}, textString = "Awall",  fontSize = 0 ), Line(origin = {0, 40}, points = {{0, -30}, {0, 30}}, color = {255, 0, 0}, thickness = 2, arrow = {Arrow.None, Arrow.Filled}), Line(origin = {0, -38}, points = {{0, 30}, {0, -30}}, color = {255, 0, 0}, thickness = 2, arrow = {Arrow.None, Arrow.Filled})}, coordinateSystem(initialScale = 0.1)),
